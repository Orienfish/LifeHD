...
if [ "$2" = "mnist" ]; then
  if [ "$3" = "iid" ]; then
    python3 main.py --method "$1" --dataset "$2" --feature_ext "$feat_ext" --hd_encoder "$5" \
      ...
      --max_classes $mem --beta $beta --k_merge_min $k_merge_min --label_ratio "$6" \
...
if [ "$2" = "mhealth" ]; then
  if [ "$3" = "iid" ]; then
    python3 main.py --method "$1" --dataset "$2" --feature_ext "$feat_ext" --hd_encoder "$5" \
      ...
      --max_classes $mem --beta $beta --k_merge_min $k_merge_min --win_size 128 --overlap 0.75 --label_ratio "$6" \
      --confidence "$confidence"
  fi
...
if [ "$2" = "esc50" ]; then
  if [ "$3" = "iid" ]; then
    python3 main.py --method "$1" --dataset "$2" --feature_ext "$feat_ext" --hd_encoder "$5" \
      ...
      --label_ratio "$6" --confidence "$confidence"
  fi
