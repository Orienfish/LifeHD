...
import numpy as np;
import random;
import esc50_opts as opts
sys.path.append('../')
import dataset.esc50_utils as U
...
if __name__=='__main__':
    ...
    for sr in [44100, 20000]:
        ...
        test_data_dir = os.path.join(esc50_path, 'test_data_{}khz'.format(sr//1000));
        print(test_data_dir)
        ...
        val_sounds = [];
        val_labels = [];
        dataset = np.load(os.path.join(esc50_path, 'wav{}.npz'.format(opt.sr // 1000)), allow_pickle=True);
        for s in opt.splits:
